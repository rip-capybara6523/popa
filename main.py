...
ALLY_TYPES = [
    ...
    {"name": "Кот крепыш", "cost": 150, "hp": 260, "speed": 1, "atk": 23, "color": CAT_TANK_COLOR, "size": 40,
     "ranged": False},
    {"name": "Танковый кот", "cost": 250, "hp": 450, "speed": 1, "atk": 33, "color": CAT_FAST_COLOR, "size": 40,
     "ranged": False},
    ...
    {"name": "Гипер кот", "cost": 500, "hp": 1600, "speed": 4.5, "atk": 48, "color": CAT_SUPER_COLOR, "size": 40,
     "ranged": False},
    {"name": "Ультра куб", "cost": 1000, "hp": 6500, "speed": 0.65, "atk": 160, "color": CAT_ULTRA_COLOR, "size": 64,
     "ranged": False},
]

ENEMY_TYPES = [
    {"hp": 120, "speed": 1, "atk": 8, "color": ENEMY_COLOR, "ranged": False, "range": 0, "name": "Враг", "level": 1,
     "size": 40},
    {"hp": 200, "speed": 1.7, "atk": 16, "color": ENEMY_FAST_COLOR, "ranged": False, "range": 0, "name": "Быстрый",
     "level": 2, "size": 40},
    {"hp": 420, "speed": 0.9, "atk": 30, "color": ENEMY_BOSS_COLOR, "ranged": False, "range": 0, "name": "Босс",
     "level": 3, "size": 40},
    {"hp": 70, "speed": 1.05, "atk": 16, "color": ENEMY_RANGED_COLOR, "ranged": True, "range": int(WIDTH // 2),
     "name": "Дальний", "level": 4, "size": 40},
    {"hp": 540, "speed": 0.66, "atk": 38, "color": ENEMY_SUPER_COLOR, "ranged": False, "range": 0, "name": "Супер-куб",
     "level": 5, "size": 56}
]
...
class EnemyBoss(Unit):
    def __init__(self, x, y):
        super().__init__(x, y, BOSS_STATS["hp"], BOSS_STATS["speed"], BOSS_STATS["atk"], BOSS_STATS["color"],
                         BOSS_STATS["size"], False, 0, "enemy")
...
class Game:
    ...
    def try_spawn_cat(self):
        ...
        if enough_energy and cooldown_ready:
            ...
            if self.selected_cat == 3:
                cat = RangedCat(self.player_base.rect.right, y_spawn, st["hp"], st["speed"], st["atk"], st["color"],
                                st["range"], st["shoot_cooldown"], st["size"])
            elif self.selected_cat == 5:
                cat = UltraCat(self.player_base.rect.right, y_spawn - 24, st["hp"], st["speed"], st["atk"], st["color"],
                               st["size"])
            else:
                cat = Unit(self.player_base.rect.right, y_spawn, st["hp"], st["speed"], st["atk"], st["color"],
                           st["size"], ranged=st.get("ranged", False), range_dist=st.get("range", 0), side='ally')
            self.cats.add(cat)
    ...
    def spawn_enemies(self):
        ...
        if now >= self.next_enemy_spawn:
            ...
            if et.get("ranged", False):
                enemy = Unit(spawn_x, y, et["hp"], et["speed"], et["atk"], et["color"], size, ranged=True,
                             range_dist=et.get("range", 0), side='enemy')
            else:
                enemy = Unit(spawn_x, y, et["hp"], et["speed"], et["atk"], et["color"], size, ranged=False,
                             range_dist=0, side='enemy')
    ...
    def remove_dead_units(self):
        """Видаляє всі мертві юніти з усіх груп"""
        dead_units = []

        for sprite in list(self.all_units):
            if sprite.is_dead():
                if sprite in self.cats:
    ...
    def update_units(self):
        ...
        # Дальний кот (не RangedCat) стреляет в врагов
        for cat in list(self.cats):
        ...
        # Кошки атакуют врагов при контакте
        for cat in list(self.cats):
            ...
            for enemy in hit_enemies:
                ...
                if not isinstance(cat, RangedCat) and not cat.ranged:
                    if enemy_id not in cat.last_attack_time:
                        cat.last_attack_time[enemy_id] = 0

                    if now - cat.last_attack_time[enemy_id] >= 0.5:
                        enemy.hp -= cat.atk
                        cat.last_attack_time[enemy_id] = now

                # Враг атакует кошку
                if not getattr(enemy, 'ranged', False) or isinstance(enemy, EnemyBoss):
                    if id(cat) not in enemy.last_attack_time:
                        enemy.last_attack_time[id(cat)] = 0

                    if now - enemy.last_attack_time[id(cat)] >= 0.5:
                        cat.hp -= enemy.atk
                        enemy.last_attack_time[id(cat)] = now

        # Кошки атакуют ворожью базу
        for cat in list(self.cats):
            if not self.boss_spawned or self.boss_defeated:
                if cat.rect.colliderect(self.enemy_base.rect):
                    if 'base' not in cat.last_attack_time:
                        cat.last_attack_time['base'] = 0

                    if now - cat.last_attack_time['base'] >= 0.5:
        ...
        for enemy in list(self.enemies):
            ...
            # Дальний враг стреляет в кошек
            if getattr(enemy, 'ranged', False) and not isinstance(enemy, EnemyBoss):
                if not hasattr(enemy, 'ranged_reload'):
                    enemy.ranged_reload = 0
                enemy.ranged_reload = max(0, enemy.ranged_reload - 1)
                if enemy.ranged_reload == 0:
            ...
            # Враги атакуют базу игрока
            if enemy.rect.colliderect(self.player_base.rect):
                enemy_id = id(enemy)
                if enemy_id not in enemy.last_attack_time:
                    enemy.last_attack_time[enemy_id] = 0

                if now - enemy.last_attack_time[enemy_id] >= 0.5:
                    self.player_base.damage(enemy.atk)
                    enemy.last_attack_time[enemy_id] = now

        # Обновляем пули
        self.update_bullets()

        # Удаляем мертвые юніти
